...
```yaml
access_control:
    - { path: ^/public-api-v1/, roles: PUBLIC_ACCESS }
...
Only the first matching rule applies. Public pages (API endpoints, sign-in, sign-up, invitation acceptance) are explicitly listed. All application features require `ROLE_USER`. The role hierarchy grants `ROLE_ADMIN` all permissions of `ROLE_USER`:
